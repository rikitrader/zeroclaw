...
The repo also ships upstream's `install.sh`, which asks whether you want a prebuilt binary (fast, ~seconds) or a source build (slower, customisable). Both end the same way: `zeroclaw quickstart` kicks off automatically.
...
We welcome contributions! See [CONTRIBUTING.md](CONTRIBUTING.md) for guidelines. Larger changes go through the [RFC process](docs/book/src/contributing/rfcs.md).
