...
pub fn create_runtime(config: &RuntimeConfig) -> anyhow::Result<Box<dyn RuntimeAdapter>> {
    match config.kind {
        ...
        // X0 fork: the WASM runtime lives in the root crate behind the
        // runtime-wasm feature; this config-level factory cannot build it.
        RuntimeKind::Wasm => anyhow::bail!(
            "runtime.kind='wasm' is handled by the zeroclaw binary's runtime factory (runtime-wasm feature), not the config-level factory."
        ),
    }
}
