...
use zeroclaw_runtime::rpc::types::{
    ...
    ConfigSectionsResult, PickerItem, PickerResponse, SelectItemResponse,
};
...
/// Alias-aware model-catalog derivation. When `alias` names a configured
/// `providers.models.<provider>.<alias>` entry, the entry's own URI/auth is
/// probed so the dashboard validates the operator's actual endpoint rather
/// than the provider family's default. Without an alias this delegates to
/// the same `quickstart::model_catalog` path the CLI wizard uses (which
/// also carries pricing when the endpoint returns it).
async fn catalog_models_for_config(
    ...
    let local = zeroclaw_runtime::quickstart::model_provider_is_local(model_provider);
    let not_live = |model_provider: &str| CatalogModelsResult {
    ...
    if let Some(alias) = alias {
        let Some(entry) = cfg.providers.models.find(model_provider, alias) else {
            return not_live(model_provider);
        };
...
/// Wire shape of `GET /api/config/status`. Mirrors the shared
/// `ConfigStatusResult` RPC type field-for-field and extends it with the
/// structured `repair_items` checklist so the dashboard can route the
/// operator straight to the broken section without parsing `missing` copy.
#[derive(Debug, Serialize)]
#[cfg_attr(feature = "schema-export", derive(schemars::JsonSchema))]
pub struct SectionStatusResponse {
    /// `true` when no agent is dispatchable yet. The dashboard uses this
    /// signal to redirect first-load visits from `/` to `/quickstart`.
    pub needs_quickstart: bool,
    /// Short machine-readable reason for the value of `needs_quickstart`,
    ...
    /// setup" instead of pretending the flow is fresh.
    pub has_partial_state: bool,
    /// Human-readable readiness failures. When setup cannot finish, the
...
/// that state.
#[must_use]
pub fn derive_section_status(cfg: &zeroclaw_config::schema::Config) -> SectionStatusResponse {
...
fn typed_family_config_key(section: Section, key: &str) -> String {
    if matches!(
        section,
...
mod tests {
    ...
    #[test]
    fn derive_section_status_returns_structured_repair_items_for_half_configured_agent() {
        let mut cfg = zeroclaw_config::schema::Config::default();
        cfg.create_map_key("providers.models.atomic_chat", "local")
            .unwrap();
        cfg.create_map_key("risk_profiles", "default").unwrap();
        cfg.create_map_key("agents", "default").unwrap();
        let agent = cfg.agents.get_mut("default").unwrap();
    ...
    async fn catalog_models_marks_unreachable_hosted_alias_endpoint_not_live() {
        ...
        cfg.create_map_key("providers.models.moonshot", "default")
            .unwrap();
        cfg.set_prop_persistent("providers.models.moonshot.default.api_key", "sk-test")
            .unwrap();
