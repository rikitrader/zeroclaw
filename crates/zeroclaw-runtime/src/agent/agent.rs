...
impl Agent {
    ...
    async fn from_config_with_session_cwd_and_mcp_approval_mode(
        ...
        let mut agent = Agent::builder()
            ...
            // `build_runner` (not `HookRunner::from_config`) so binary-local
            // registry extras — the X0 conscience gate, continuity — are
            // included alongside the built-in hooks. Returns `None` when
            // `hooks.enabled = false`.
