...
pub async fn run_tool_call_loop(p: ToolLoop<'_>) -> Result<String> {
    ...
    for iteration in 0..max_iterations {
        ...
        // ── Hook: before_llm_call (modifying) ─────────────────
        // Hooks may adjust the message history before the request is built
        // (e.g. continuity injects learned preferences as a leading system
        // message). A cancelling hook aborts the turn with its reason.
        if let Some(hooks) = hooks {
            // Hooks may rewrite the model string; the loop's provider binding
            // is fixed for the turn, so a model rewrite is recorded but not
            // applied (same as the pre-extraction loop, which discarded it).
            let mut hook_model = model.to_string();
            match hooks.run_before_llm_call(history, &mut hook_model).await {
