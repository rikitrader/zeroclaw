...
pub async fn run(
    ...
    if let Some(gateway_start) = registry.take_gateway_start() {
        ...
        for (alias, agent) in &config.agents {
            ...
            let agent_reload_controls = GatewayReloadControls {
                shutdown_tx: gateway_shutdown_tx.clone(),
                reload_tx: reload_tx.clone(),
            };
            let agent_tui_registry = tui_registry.clone();
            ...
            handles.push(spawn_component_supervisor(
                ...
                move || {
                    ...
                    let reload_controls = agent_reload_controls.clone();
                    let tui_reg = agent_tui_registry.clone();
