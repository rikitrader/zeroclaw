...
export default function AgentCard({ agent, onSelect, selected = false }: AgentCardProps) {
  ...
  // Dedicated per-agent gateway (PR #10): open its URL without triggering
  // the row's detail drawer. Rendered as a span+role, not an <a>, because
  // the whole row is a <button> and nested anchors are invalid HTML.
  const openGateway = (e: { stopPropagation: () => void; preventDefault: () => void }) => {
    e.stopPropagation();
    e.preventDefault();
    if (agent.gatewayUrl) window.open(agent.gatewayUrl, '_blank', 'noopener,noreferrer');
  };
  ...
  return (
    ...
    >
      ...
      {/* Gateway info — dedicated per-agent gateway (PR #10) gets an
          "isolated" chip that opens its URL; shared agents get a muted
          "shared :port" chip. Hidden when no gateway info has been resolved
          (older daemons without /api/agents/summary). */}
      {agent.gatewayPort !== null &&
        (agent.dedicatedGateway && agent.gatewayUrl ? (
          <span
            ...
            onClick={openGateway}
            onKeyDown={(e) => {
              if (e.key === 'Enter' || e.key === ' ') openGateway(e);
            }}
            className="hidden sm:inline-flex items-center gap-1 h-6 px-2 rounded-full flex-shrink-0 text-xs font-medium tabular-nums cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--pc-focus)]"
            style={{
          ...
          >
            ...
            isolated · :{agent.gatewayPort}
            <ExternalLink className="h-2.5 w-2.5" />
          </span>
        ) : (
          <span
            className="hidden sm:inline-flex items-center gap-1 h-6 px-2 rounded-full flex-shrink-0 text-xs tabular-nums text-pc-text-secondary bg-pc-elevated"
